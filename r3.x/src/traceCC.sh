...
function usage ()  {
    echo "traceCC.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t     created by <lgyalog@avaya.com>"	
	echo 'Usage: traceCC.sh [OPTIONS] [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the CCMS_SGM_SipMessages.log file from the Contact Center (CC) server"
	echo -e "\t<folder>\tincludes either of the above files"	
	echo '  Options:'
...
if [[ $((dirdefined)) == 0 ]]; then	
	...
	if [[ $((dirdefined)) != 0 ]]; then
		if [[ $((vsyslog)) == 10 ]]; then
			ip=$(awk -F"Address IP: " '{print $2}' <<< "$line")
			ip1=$(cut -d':' -f1 <<< "$ip")
			ip2=$(cut -d':' -f2 <<< "$ip" | cut -d')' -f1)
       		ip="$ip1:$ip2"
		elif [[ $((vsyslog)) == 11 ]]; then
			ip=$(awk '{print $NF}' <<< "$line")					# TODO: strip off ^M
			ip1=$(cut -d':' -f1 <<< "$ip")
			ip2=$(awk -F":" '{printf "%i",$2}' <<< "$ip")  	# cut -d':' -f2)
			ip="$ip1:$ip2"
		fi
	fi
fi	
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -t1 CCMS_SGM_SipMessages*.log 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
...
#			targetfiles=$(head -1 <<< $targetX)
			targetfiles=${targetX%% *}							# first word
		fi
	fi
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [[ $file == "" ]]; then
		echo -e "\nerror: could not find any CC related logs in $var\n"
		error=1
	fi
	cd "$currdir"
else
...
if [[ $file != "" ]] && [ -s "$file" ]; then
	...
	if [[ $rec == 0 ]] || [[ $rec2 == 0 ]];	then
		error=1
		echo -e "\nerror: No SIP messages have been found in $var in the expected format."
		echo "This file may not be a CC CCMS_SGM_SipMessages.log file... or, DEBUG was not enabled."
#		rec=$(egrep -c -e "^CSeq:.*" $file)
		if [[ $rec2 == 0 ]]; then
			echo "In fact, no sign of any "CSeq:" lines within $basefile"
			error=2
		else
			echo "Though, found $rec2 lines with "CSeq:" - so there might be some SIP messages within $basefile."
			rec=0; 	error=2
...
#			if [[ $((n)) != 0 ]]; then				
				...
				echo -e "\nerror: could not find any "message sent/received" lines in $var"
...
#		conv=$(awk -W source='/--> [Mm]essage|<-- [Mm]essage/{flag=1} flag; /}/{flag=0}' "$file")
		if [[ $((vsyslog)) == 10 ]]; then
			conv=$(awk -W source='/begin_record/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
			if [[ $? != 0 ]]; then
				conv=$(awk -e '/begin_record/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
			fi
		elif [[ $((vsyslog)) == 11 ]]; then
			conv=$(awk -W source='/essage sent|essage received/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
			if [[ $? != 0 ]]; then
				conv=$(awk -e '/essage sent|essage received/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
			fi
		fi				
...
#		    if [[ $line == *"begin_record"* ]]; then
			if [[ $line =~ \-\-\>\ [Mm]essage|\<\-\-\ [Mm]essage ]]; then
				...
				if [[ $ip != "" ]] && [[ $endptaddr != "" ]] && [[ $ip == *$endptaddr* ]]; then
					...
					line=$(awk -F"Start-Line: " '{print $2}' <<< "$line")
					start_sipmsg
				fi
		...
		if [[ $((error)) == 0 ]] && [[ $((bCAT)) != 0 ]] && [[ $((n)) != 0 ]]; then
			...
			echo "Converted $file into $output.asm, and concatenated it into $ctarget."
			echo ''; ls -l "$ctarget"				
			echo ''
		fi
	fi
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -s "$var" ] && [[ $filetype =~ text ]]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "CC" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"
...
#		nfiles=$(wc -w <<< "$filelist")
		...
		if [[ $((nfiles)) -gt 1 ]]; then
			echo -e "\nWarning: about to convert multiple files ($nfiles x CCMS_SGM_SipMessages.log*.txt)."

...
function usage ()  {
    echo "traceIVIEW.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t\tcreated by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceIVIEW.sh [OPTIONS] [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the sipserver.log (Scopia) or sip.log file found in iVIEW_CS_CAPTURE,"
	echo -e "\t\t\tor the logreport itself (.zip/.tgz/.tar) from an IX Meetings server"
...
if [[ $((dirdefined)) != 0 ]]; then
	if [[ $foundipddr != "" ]] && [[ $endptaddr != "" ]] && [[ $foundipaddr != *$endptaddr* ]]; then
		...
		if [[ $((sipstat)) != 0 ]]; then
			if [[ $bvar != "" ]]; then
				echo -en "$bvar => $n/$rec Msgs converted             \r"
			else
...
#	if [[ $((dirdefined)) != 0 ]]; then
	if [[ $((vsyslog)) == 8 ]]; then
 		ip=$(cut -d' ' -f6 <<< "$line")
		siplength=$(awk '{ printf "%i",$8}' <<< "$line") 						 # cut -d' ' -f8)
	else
		ip="6.6.6.6:6666"
		siplength=666
	fi
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -t1 sip[1-9].log 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
	...
	targetX=""; targetX=$(ls -t1 sip.log.10 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
	...
	targetX=""; targetX=$(ls -t1 sip.log.[1-9] 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
	...
	targetX=""; targetX=$(ls -t1 sip.log 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
			targetfiles="$targetfiles $targetX"
		else
	...
	targetX=""; targetX=$(ls -t1 sipserver.log 2>/dev/null)						# for Scopia server
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
			targetfiles="$targetfiles $targetX"
		else
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	explore_logfolder
	if [[ $file == "" ]]; then
		echo -e "\nerror: could not find any iView related logs in $folder\n"
		error=1
	fi
	cd "$currdir"
else
...
if [[ $file != "" ]] && [ -s "$file" ]; then
	...
	xtype=$(file -b "$file")
	if [[ $xtype =~ data ]]; then
		echo "ALERT: Found binary data in $file. Attempting to remove lines with binary data ..."
# 0000260    5345    4153    4547    205d    8b1f    0a08
#           E   S   S   A   G   E   ]     037 213  \b  \n
# x1f or 0x8 is good, 0x8b not good for filtering
# - alternative binary removal: tr -d '\200-\377'
		grep -avP "\x1f" < "$file" > "$file.TEXT"									# egrep will fail with msg: grep: conflicting matchers specified
		file="$file.TEXT"; tmpfile=1
		xtype=$(file -b "$file")
		if [[ $xtype =~ data ]]; then
			echo "error: some binary data remained in $file.  The script may not complete the conversion due to remaining binary data. Contact developer."
		fi
	fi
...
#	rec=$(egrep -c -e ".*MESSAGE\] <---.*|.*MESSAGE\] --->.*" <$file)
	...
	if [[ $rec != 0 ]]; then  # iVIEW sip log
		...
		file="$file.MESSAGE"; tmpfile=1
		vsyslog=8
	else
		rec=$(egrep -m 1 -c -e ".*tSipAdap.*TRANSPORT.* CSeq:*" "$file")
		if [[ $rec != 0 ]]; then
			rec=0; error=1
			echo -e "\nerror: $basefile appears to be an MCU logfile - try to use \"traceMCU.sh\" script instead."
		else
			echo -e "\nerror: No SIP messages have been found in $basefile in the expected format."
			echo "This file may not be an iVIEW sip.log file... or, DEBUG loglevel was not enabled."
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "IVIEW" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"
...
#		nfiles=$(wc -w <<< "$filelist")
		...
		if [[ $((nfiles)) -gt 1 ]]; then
			echo -e "\nWarning: about to convert multiple files ($nfiles x sip.log*.txt)."

...
# TODO: is there a logreport for ADA??

function usage ()  {
    echo "traceADA.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t      created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceADA.sh [OPTIONS] [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the dsa.log file from an ADA server debug log"
	echo -e "\t\t\tor a cereport.tgz file collected from Breeze server (with DeviceAdapter logs)"			
...
# Dec 15 09:02:08 greenedpeccm dsa: DEBUG: [21129] CSDK << SIPSENDING 1099 bytes to 135.9.49.218:5091 {
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -r dsa.log.[0-9]* 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
	...
	targetX=""; targetX=$(ls dsa.log 2>/dev/null)
	if [ $? == 0 ] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [ -d "var/log/Avaya/services/DeviceAdapter" ]; then
		if [ -d "var/log/Avaya/sm" ]; then
			...
			if [[ $((attdfiles)) != 0 ]]; then
				echo "NOTE: found $attdfiles Attendant.log files at \"var/log/Avaya/services/Attendant\" folder."
				echo  "These files can be decoded using traceATTD.sh script."
			fi
		fi

		destdir="$destdir/var/log/Avaya/services/DeviceAdapter"
		target="$target-ADA"
		cd "var/log/Avaya/services/DeviceAdapter"

	elif [ -d "log/Avaya/services/DeviceAdapter" ]; then
		if [ -d "log/Avaya/sm" ]; then
			...
			let attdfiles=0
			attdfiles=$(ls log/Avaya/services/Attendant/Attendant* 2>/dev/null | wc -l)
			if [[ $((attdfiles)) != 0 ]]; then
				echo "NOTE: found $attdfiles Attendant.log files at \"log/Avaya/services/Attendant\" folder."
				echo "These files can be decoded using traceATTD.sh script."
			fi
		fi

		destdir="$destdir/log/Avaya/services/DeviceAdapter"
		target="$target-ADA"
		cd "log/Avaya/services/DeviceAdapter"

	elif [ -d "Avaya/services/DeviceAdapter" ]; then
		if [ -d "Avaya/sm" ]; then
			let asmfiles=0
			...
			let attdfiles=0
			attdfiles=$(ls Avaya/services/Attendant/Attendant* 2>/dev/null | wc -l)
			if [[ $((attdfiles)) != 0 ]]; then
				echo "NOTE: found $attdfiles Attendant.log files at \"Avaya/services/Attendant\" folder."
				echo "These files can be decoded using traceATTD.sh script."
			fi
		fi

		destdir="$destdir/Avaya/services/DeviceAdapter"
		target="$target-ADA"
		cd "Avaya/services/DeviceAdapter"

	elif [ -d "services/DeviceAdapter" ]; then
		if [ -d "sm" ]; then
			let asmfiles=0
			asmfiles=$(ls sm/asm* 2>/dev/null | wc -l)
			if [[ $((asmfiles)) != 0 ]]; then
				echo "NOTE: found $asmfiles asm files at \"sm\" folder."
				echo "These files can be decoded using traceBREEZE.sh script."
			fi
		elif [ -d "services/Attendant" ]; then
			let attdfiles=0
			attdfiles=$(ls services/Attendant/Attendant* 2>/dev/null | wc -l)
			if [[ $((attdfiles)) != 0 ]]; then
				echo "NOTE: found $attdfiles Attendant.log files at \"services/Attendant\" folder."
				echo "These files can be decoded using traceATTD.sh script."
			fi
		fi

		destdir="$destdir/services/DeviceAdapter"
		target="$target-ADA"
		cd "services/DeviceAdapter"

	elif [ -d "DeviceAdapter" ]; then
		if [ -d "sm" ]; then
			let asmfiles=0
			asmfiles=$(ls sm/asm* 2>/dev/null | wc -l)
			if [[ $((asmfiles)) != 0 ]]; then
			...
			let attdfiles=0
			attdfiles=$(ls Attendant/Attendant* 2>/dev/null | wc -l)
			if [[ $((attdfiles)) != 0 ]]; then
				echo "NOTE: found $attdfiles Attendant.log files at \"Attendant\" folder."
				echo "These files can be decoded using traceATTD.sh script."
			fi
		fi

		destdir="$destdir/DeviceAdapter"
		target="$target-ADA"
		cd "DeviceAdapter"

	elif [ -d "Device Adapter" ]; then
		destdir="$destdir/Device Adapter"
		target="$target-ADA"
		cd "Device Adapter"
	...
	if [[ $file == "" ]]; then
		error=1
		echo -e "\nerror: could not find any ADA related logs in this folder: $folder\n"
	fi
	cd "$currdir"		
else
...
#	echo "                                                                                                                                                  "
	...
	rec=$(egrep -c -e ".*SIPSENDING.*|.*SIPRECEIVED.*" "$file")

	if [[ $rec == 0 ]];	then
		echo -e "\nerror: No SIP messages have been found in $basefile in the expected format."
		echo "This file may not be a DeviceAdapter dsa.log file... or, DEBUG was not enabled."
		...
		do
		    ...
		    if [[ $line == *"SIPRECEIVED"* ]] || [[ $line == *"SIPSENDING"* ]]; then
			    if [[ $((sipstart)) != 0 ]]; then
			    	complete_sipmsg
			    fi
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "ADA" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"

...
function usage ()  {
    echo "tracePhoner.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t\tcreated by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: tracePhoner.sh [OPTIONS] [<LOG_FILE>,...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the debugSIP.txt file (collected from Phoner or PhonerLite 3rd-party softclient)"	
	echo '  Options:'
...
if [[ $((dirdefined)) == 0 ]]; then		
	...
	if [[ $((dirdefined)) != 0 ]]; then
       ip=$(cut -d' ' -f3 <<< "$line")
	   protocol=$(cut -d' ' -f4 <<< "$line" | cut -d'(' -f2 | cut -d')' -f1)
	fi
fi	
} # sip_direction()

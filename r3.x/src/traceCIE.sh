...
function usage ()  {
    echo "traceCIE.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t      created by <lgyalog@avaya.com>"	
	echo 'Usage: traceCIE.sh [OPTIONS] [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
...
#          siplength=$(echo "$line" | cut -d'=' -f2)  				# TODO: strip off ^M character
		   siplength=$(awk -F'=' '{printf "%i",$2}' <<< "$line")  	# 'awk' strips off ^M character
	   fi
	fi
fi	
} # sip_direction()
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -t1 2*chap*.log 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [ -d "Avaya/LogFiles/TTLogFiles/ACM-IPO" ]; then
		target="$target-CIE"		
		destdir="$destdir/Avaya/LogFiles/TTLogFiles/ACM-IPO"
		cd "Avaya/LogFiles/TTLogFiles/ACM-IPO"
	elif [ -d "LogFiles/TTLogFiles/ACM-IPO" ]; then
		target="$target-CIE"		
		destdir="$destdir/LogFiles/TTLogFiles/ACM-IPO"
		cd "LogFiles/TTLogFiles/ACM-IPO"
	elif [ -d "TTLogFiles/ACM-IPO" ]; then
		target="$target-CIE"		
		destdir="$destdir/TTLogFiles/ACM-IPO"
		cd "TTLogFiles/ACM-IPO"
	elif [ -d "ACM-IPO" ]; then
		target="$target-CIE"		
		destdir="$destdir/ACM-IPO"
		cd "ACM-IPO"
	...
	explore_logfolder
	if [[ $file == "" ]]; then
		echo -e "\nerror: could not find any CIE related logs in $folder\n"
		error=1
	fi
	cd "$currdir"
else
...
#	echo "                                                                                                                                                  "
	...
	if [[ $rec == 0 ]]; then
		...
		echo "This file may not be a CIE chap.log file... or, DEBUG was not enabled."
		error=1; rec2=$(egrep -c -e "^CSeq:*" "$file")	
#		rec=$(egrep -c -e "^CSeq:.*" "$file")
		...
		if [[ $rec -gt 1000 ]]; then
			echo "Preparing $basefile for processing ..."
		fi

		conv=$(awk -W source='/CHAP_SIP_Message/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
		if [[ $? != 0 ]]; then
			conv=$(awk -e '/CHAP_SIP_Message/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
		fi
...
#  - d:\Avaya\LogFiles\TTLogfiles\ACM-IPO\2016_05_16_tt_[chap@mss-avayacie].log
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "CIE" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"

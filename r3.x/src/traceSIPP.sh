...
function usage ()  {
    echo "traceSIPP.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t       created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceSIPP.sh [OPTIONS] [<LOG_FILE>,...]'
	echo '  Where:'
	...
	echo -e "\n  Note:\t\t\tSIPP does not log IP addr of sender/receiver, therefore using 1.1.1.1 and 6.6.6.6\n"
...
if [[ $((dirdefined)) == 0 ]]; then		
	if [[ $line == *" message received "* ]]; then
	    ...
	    siplength=$(cut -d' ' -f4 <<< "$line" | cut -d'[' -f2)
...
for var in "$@"
	...
	if [ -f "$file" ]; then
		...
		rec2=$(egrep -c -m 1 -e "^----------------------------------------------- [12][0-9]{3}.*" < "$file" 2>/dev/null)
#		rec3=$(egrep -c -m 1 -e "sipp")

        if [[ $rec2 == 0 ]]; then
		   echo "error: $bvar is not an messages.log created by SIPP tool"
		   ...
		   error=1; continue
			...
			echo "error: no SIP messages have been found in $bvar"
		    echo "Verify source and content of $bvar."			
			error=2; continue
		else
			logsec=$SECONDS

...
#	echo ''
	echo 'Usage: traceASM62.sh [OPTIONS] [<LOG_FILE> | <sm-report> | <folder>, ...]'
	echo '  Where:'
	...
	echo -e "\t-u URI|NUMBER\tfilter messages that contain <URI|NUMBER>"
...
if [[ $((dirdefined)) == 0 ]]; then	
	...
	*)	insidesip=0; dirdefined=0;;
	esac
else
	echo -e "\nAlert: sip_direction() was called with dirdefined=$dirdefined at time=$siptime, line#$nlines.  Contact developer."	
fi	
} # sip_direction()
...
# 2012-10-01 15:43:14,862 CallLogs INFO - : Outgoing Message
	case $vsyslog in
	1)												 			# ASM6.2 SipTrace CallLogs INFO
	...
	5)															# or, sipdate= | cut -d' ' -f16| cut -d'>' -f3 , siptime= | cut -d' ' -f17
...
do
	...
	if [[ $((vsyslog)) == 5 ]]; then
		...
		echo -e "# msgno: $n" >> "$newfile"
		echo -en "$n => $rec Msgs converted            \r"				
#		if [[ $((insidesip)) == 1 ]]; then 	echo -e "--------------------" >> "$newfile"; fi
...
if [[ $# -eq 0 ]]; then
  ...
  while getopts ":ae:i:hbf:su:N:" options; do
	case "${options}" in
	h)
		usage
		exit 0;;
...
for var in "$@"
	...
	if [ -s "$file" ]; then
		...
		if [[ $rec != 0 ]]; then
			echo -e "\nNo conversion required. $bvar appears to be a native traceSM capture."
		...
		if [[ $rec == 0 ]];	then
			...
			if [[ $rec == 0 ]]; then
				...
				error=1; asmfile=0; asmfile=$(egrep -m 1 -c "SIPMSGT" "$file" 2>/dev/null)		
				if [[ $((asmfile)) != 0 ]]; then
...
#				elif [[ $xline =~ ^[ADFMJSON][a-z][a-z].* ]] && [[ $xline =~ .*\]:\ $ ]]; then
		...
		if [[ $conv2asm == 1 ]]; then
			...
			if [ -f "$newfile" ]; then rm $newfile 2>/dev/null; fi
					...
					if [[ $line == "--------------------"* ]]; then				
				       complete_sipmsg
						...
						if [[ $((base64decode)) != 0 ]] && [[ $line == "Base64 dump"* ]]; then
							...
							if [[ -f "$newfile.b64" ]]; then rm "$newfile.b64" 2>/dev/null; fi

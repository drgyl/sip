...
function usage ()  {
    echo "traceCBA.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t      created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceCBA.sh [OPTIONS] [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the siptrace.log file from the CBA server's BSR log"
...
# time="06-10-2017 02:54:39,774"
	if [[ $timestamp == *"-"* ]]; then
		sipyear=$(cut -d' ' -f1 <<< "$timestamp")
		sipmonth=$(cut -d'-' -f1 <<< $sipyear)
		sipday=$(cut -d'-' -f2 <<< $sipyear)
		sipyear=$(cut -d'-' -f3 <<< $sipyear)

		sipmsec=$(cut -d' ' -f2 <<< "$timestamp")
...
# time="1487794945966"	
		let xtime=$((timestamp))/1000
		sipyear=$(date -d @$xtime +'%m/%d/%Y %H:%M:%S')
		sipmsec=$(cut -d' ' -f2 <<< "$sipyear")
		sipmonth=$(cut -d'/' -f1 <<< "$sipyear")
		sipday=$(cut -d'/' -f2 <<< "$sipyear")	
		sipyear=$(cut -d'/' -f3 <<< "$sipyear" | cut -d' ' -f1)	

		siphour=$(cut -d':' -f1 <<< "$sipmsec")
		sipmin=$(cut -d':' -f2 <<< "$sipmsec")
		sipsec=$(cut -d':' -f3 <<< "$sipmsec")

		sipmsec=$(echo $(($timestamp % 1000)))
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -r -t1 siptrace[1-9].log 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
	...
	targetX=""; targetX=$(ls -r -t1 siptrace.*.log 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		if [[ "$targetfiles" != "" ]]; then
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [ -d "cbalogfiles" ]; then
		destdir="$destdir/cbalogfiles"
		cd "cbalogfiles"
	fi

	if [ -d "bsr-server" ]; then
		destdir="$destdir/bsr-server"
		target="$target-CBA"
		cd "bsr-server"
	...
	if [[ $file == "" ]]; then
		error=1
		echo -e "\nerror: could not find any CBA related logs in $folder\n"
	fi
else
...
#	echo "                                                                                                                                                  "
	rec=$(egrep -c "^<message" "$file")
#	rec2=$(egrep -c -e "^CSeq:*" "$file")		

	if [[ $rec == 0 ]];	then
		error=1
		echo -e "\nerror: No SIP messages have been found in $basefile in the expected format."
		echo "This file may not be a CBA BSR siptrace.log file... or, DEBUG was not enabled."
...
#		conv=$(awk -e '/CHAP_SIP_Message/{flag=1} flag; /}/{flag=0}' "$file")
		conv=$(awk -W source='/^<message/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
		if [[ $/ != 0 ]]; then
			conv=$(awk -e '/^<message/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
		fi
		...
		do
			...
			elif [[ $((insidesip)) == 1 ]] && [[ $line =~ ^from= ]]; then
			    insidesip=2
				ip=$(cut -d'"' -f2 <<< "$line")

			elif [[ $((insidesip)) == 2 ]] && [[ $line =~ ^to= ]]; then		
			    insidesip=3
				localip=$(cut -d'"' -f2 <<< "$line")

			elif [[ $((insidesip)) == 3 ]] && [[ $line =~ ^time= ]]; then
				insidesip=4
				timestamp=$(cut -d'"' -f2 <<< "$line")

			elif [[ $((insidesip)) == 4 ]] && [[ $line =~ ^timeStamp ]]; then
				insidesip=5

			elif [[ $((insidesip)) == 5 ]] && [[ $line =~ ^isSender= ]]; then
				insidesip=6
				if [[ $line =~ true ]]; then
					dirdefined=2; sip_direction
					ip1=$localip; localip=$ip; ip=$ip1						
				elif [[ $line =~ false ]]; then
					dirdefined=1; sip_direction						
				fi

			elif [[ $((insidesip)) == 6 ]] && [[ $line =~ ^firstLine= ]]; then
				insidesip=7
			...
			elif [[ $((insidesip)) == 7 ]] && [[ $line =~ ^\<\!\[CDATA ]]; then
				insidesip=8

			elif [[ $((insidesip)) == 8 ]] && [[ $((sipstart)) == 1 ]]; then
...
for var in "$@"
	...
	currdir=$PWD; currtime=$(date +%R:%S)
	bSinglefile=0; 			tmpfile=0
	filetype2=""; 			filecontent="CBA"
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "CBA" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"
...
#		nfiles=$(wc -w <<< "$filelist")
		...
		if [[ $((nfiles)) -gt 1 ]]; then
			echo -e "\nWarning: about to convert multiple files ($nfiles x siptrace.log)."

...
function usage ()  {
    echo "traceCU360.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t\tcreated by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceCU360.sh [OPTIONS] [<LOG_FILE> | <folder>,...]'
	echo '  Where:'
	...
	echo ''
	echo -e "  Note:\t\t\tthis device does not log SIP messages in completeness, also src/dest IP is unknown"
	echo''
} # usage()
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls user.log.[1-9] 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
	...
	targetX=""; targetX=$(ls -t1 user.log 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [[ $file == "" ]]; then
		error=1
		echo -e "\nerror: could not find any CU360 related logs in $folder\n"
	fi
else
...
#	echo "                                                                                                                                                  "
	...
	if [[ $rec == 0 ]]; then
		error=1;		
		echo -e "\nerror: $basefile is not an user.log file from a CU360 device..."
		if [[ $file =~ \.asm$ ]] || [[ $file =~ \.casm$ ]] || [[ $file =~ \.asm\.bak$ ]]; then
			...
			echo -e "Verify source and content of $var.\n"
		fi
	else
...
#		rec=$(egrep -c -e ".*recv <--.*|.*message sent to.*" < $file)
		...
		if [[ $rec == 0 ]]; then
			echo -e "\nerror: No SIP messages have been found in $basefile."
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "CU360" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"

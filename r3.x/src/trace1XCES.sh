...
#		conv=$(awk -e '/.*onexsip .*Received .*|.*onexsip .*Sending SIP.*/{flag=1} flag; /}/{flag=0}' "$file")
		conv=$(awk -W source='/.*onexsip .*Received .*|.*onexsip .*Sending SIP.*/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
		if [[ $? != 0 ]]; then
			conv=$(awk -e '/.*onexsip .*Received .*|.*onexsip .*Sending SIP.*/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
		fi

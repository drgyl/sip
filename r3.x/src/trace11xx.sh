...
#			siphour=0
			...
			if [[ $((vsyslog)) == 11 ]]; then 
				conv=$(awk -W source='/^Rec #/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
				if [[ $? != 0 ]]; then
					conv=$(awk -e '/^Rec #/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
				fi
			elif [[ $((vsyslog)) == 10 ]]; then
    	    	conv=$(awk -W source='/^\[[SR][NC][DV]\]/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
				if [[ $? != 0 ]]; then
    	    		conv=$(awk -e '/^\[[SR][NC][DV]\]/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)				
				fi
			fi

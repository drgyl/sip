...
#	echo ''
	...
	echo -e "  May also set IPdest=0.0.0.X:0 when sending SIP messages over Syslog (could be fixed in later FW releases)"	
...
if [[ $((dirdefined)) == 0 ]]; then		
	if [[ $line == *"Received "* ]]; then
		...
		3)		dirstring1="-->"; 	dirstring2="ingress";;		# TODOL may need to reverse it like all others
		esac
#			dirstring1="-->"
#			dirstring2="ingress"
		...
		3)	dirstring1="<--"; 		dirstring2="egress";;		# dirstring2="egress";;	TODO: may need to reverse it
		esac
...
if [[ $# -eq 0 ]]; then
  ...
  while getopts ":e:hbdf:sN:" options; do
	case "${options}" in
	h)
		usage
		exit 0;;
...
#					vsyslog=13
		...
		if [[ $rec != 0 ]] && [[ $((vsyslog)) != 0 ]]; then
			...
			if [[ $((vsyslog)) != 10  ]]; then					
				conv=$(awk -W source='/\[SIP message /{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
				if [[ $? != 0 ]]; then
					conv=$(awk -e '/\[SIP message /{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
				fi
			else   	    	
    	    	conv=$(awk -W source='/ bytes)/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
				if [[ $? != 0 ]]; then
					conv=$(awk -e '/ bytes)/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
				fi
			fi

			check=$(egrep -e "<1[3567][34567]>" < "$file" 2>/dev/null | wc -l)

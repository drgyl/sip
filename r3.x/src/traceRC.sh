...
function usage ()  {
    echo "traceRC.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t     created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceRC.sh [OPTIONS] [<LOG_FILE>,...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the smc.txt logfile (collected from ACO/RingCentral server)"
	echo '  Options:'
...
if [[ $((dirdefined)) == 0 ]]; then		
	...
	if [[ $((dirdefined)) != 0 ]]; then
       ip1=$(cut -d'|' -f5 <<< "$line" | cut -d'[' -f2)
	   siplength=$(cut -d' ' -f2 <<< "$ip1" | cut -d'(' -f2 | cut -d')' -f1)
	   protocol=$(cut -d']' -f1 <<< "$ip1" | cut -d':' -f1 )
	   ip2=$(cut -d']' -f1 <<< "$ip1" | cut -d':' -f3)
	   ip1=$(cut -d':' -f2 <<< "$ip1")
	   ip=$ip1:$ip2	   
	fi
fi
...
# |08:15:35.340|tSip#14.15368       |SIP_MSG      |<<<Tx SIP request dst[UDP:10.62.180.95:5060] (1155)
    if [[ $((n)) == 0 ]]; then
...
if [[ $# -eq 0 ]]; then
  ...
  while getopts ":e:hbsf:N:" options; do
	case "${options}" in
	h)
		usage
		exit 0;;
...
for var in "$@"
	...
	if [ -s "$file" ]; then
		...
		n=$(egrep -c -m 1 -e "SIP_MSG"  "$file" 2>/dev/null)

		if [[ $rec == 0 ]];	then
			echo "error: No SIP messages have been found in $bvar in the expected format."
			echo "This file may not be an smc.txt logfile from RingCentral... or, FINEST debug was not enabled."
	        ...
	        echo "error: $var does not appear to be an RC smc logfile."
			echo -e "Verify source and content of $bvar.\n"
			asmfile=0; asmfile=$(egrep -m 1 -c "SIPMSGT" "$file" 2>/dev/null)		
			if [[ $((asmfile)) != 0 ]]; then
				asmfile=$(egrep -m 1 -c -e "(egress|ingress):\ \{" "$file" 2>/dev/null)
...
#           conv=$(awk -e '/^\-\-\-\-\-\-\-\-\-\-\-\-\-.*\-\-\-\-\-\-\-$/{flag=1} flag; /}/{flag=0}' "$file")
			...
			do
                ...
                elif [[ $((insidesip)) == 1 ]] && [[ $line == *"|_SIP_MSG_" ]]; then
				   insidesip=2
				elif [[ $((insidesip)) == 2 ]] && [[ $((sipstart)) == 0 ]]; then
				   line=$(cut -d'|' -f5 <<< "$line")
				   sipmsg_header
	               start_sipmsg
				elif [[ $((sipstart)) == 1 ]] && [[ $line == *"|SIP_MSG"* ]]; then
				    line=$(cut -d'|' -f5 <<< "$line")
					if [[ $line == "_________" ]]; then
				       complete_sipmsg
					else

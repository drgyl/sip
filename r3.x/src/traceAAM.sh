...
# TODO: logreport (.gz/.tgz) processing

function usage ()  {
    echo "traceAAM.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t      created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceAAM.sh [OPTIONS] [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis a CSTRACE logfile from an AAM server logreport (.zip/.tar/.tgz/.gz)"
...
if [[ $((dirdefined)) == 0 ]]; then		
# 12/07 11:32:53.469802 CH061: SIP: call: SG::OnSipIncomgMsg sipio(i)<SIP/2.0 100 Trying                                          > len=429 from=10.230.1.14:5060	
	if [[ $line == *"OnSipIncomgMsg"* ]]; then
		...
		siplength=$(awk -F'len=' '{print $2}' <<< "$line" | cut -d' ' -f1)

		##ip=$(echo $line | awk '{print $5}')
	elif [[ $line == *"SendToNetwork"* ]]; then
...
# 12/07 11:24:10.732834 CH040: SIP: call: SG::OnSipIncomgMsg sipio(i)<ACK sip:10.230.1.12:5060;transport=tcp SIP/2.0              > len=1337 from=10.230.1.14:58118
#	if [[ $((vsyslog)) == 10 ]]; then 
##		if [[ $((n)) == 0 ]]; then
##			foundipaddr=$(echo $line | cut -d' ' -f5)
#			sipyear=$(echo $today     | cut -d'/' -f3)
			sipmonth=$(cut -d' ' -f1 <<< "$line")
			sipday=$(cut -d'/' -f2 <<< "$sipmonth")
			sipmonth=$(cut -d'/' -f1 <<< "$sipmonth")
##		fi

		sipsec=$(cut -d' ' -f2 <<< "$line") 
		sipmsec=$(awk -F'.' '{printf "%3i",$2/1000}' <<< "$sipsec")
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -r -t1 cstrace.* 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [ -d "var/log/avaya/messaging/cs" ]; then
		destdir="$destdir/var/log/avaya/messaging/cs"
		target="$target-AAM"
		cd "var/log/avaya/messaging/cs"

	elif [ -d "log/avaya/messaging/cs" ]; then
		destdir="$destdir/log/avaya/messaging/cs"
		target="$target-AAM"
		cd "log/avaya/messaging/cs"

	elif [ -d "avaya/messaging/cs" ]; then
		destdir="$destdir/avaya/messaging/cs"
		target="$target-AAM"
		cd "/avaya/messaging/cs"

	elif [ -d "messaging/cs" ]; then
		destdir="$destdir/messaging/cs"
		target="$target-AAM"
		cd "messaging/cs"
	...
	explore_logfolder
	if [[ $file == "" ]]; then
		echo -e "\nerror: could not find any AAM related logs in $folder\n"
		error=1
	fi
	cd "$currdir"
else
...
#	echo "                                                                                                                                                  "
	...
	rec=$(egrep -c -e ".*OnSipIncomgMsg.*len=.*|.*SendToNetwork.*len=.*" "$file")

	if [[ $rec == 0 ]];	then
		echo "error: No SIP messages have been found in $basefile in the expected format."
		echo "This file is not an AAM CSTRACE logfile... or, DEBUG loglevel was not enabled."
		...
		conv=$(awk -W source='/.*OnSipIncomgMsg.*|.*SendToNetwork.*/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
		if [[ $? != 0 ]]; then
			conv=$(awk -e '/.*OnSipIncomgMsg.*|.*SendToNetwork.*/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)		
		fi
		...
		do
			...
			if [[ $((sipstart)) != 0 ]]; then
				if [[ $line != *"OnSipIncomgMsg"* ]] && [[ $line != *"SendToNetwork"* ]]; then
					complete_sipmsg
				elif [[ $line == *"> len="* ]]; then 
					complete_sipmsg				
				fi
			...
			if [[ $((dirdefined)) == 0 ]]; then
				if [[ $line == *"OnSipIncomgMsg"* ]] && [[ $line == *"from="* ]]; then
					sip_direction
					get_sip_datetime

				elif [[ $line == *"SendToNetwork"* ]] && [[ $line == *"to="* ]]; then
					sip_direction
					get_sip_datetime
				fi
				...
				if [[ $((dirdefined)) != 0 ]]; then 
					if [[ $ip != "" ]] && [[ $endptaddr != "" ]] && [[ $ip == *$endptaddr* ]]; then
						...
						line=$(cut -d'<' -f2 <<< "$line" | cut -d'>' -f1)
						sipmsg_header
						start_sipmsg
					fi
				fi
			elif [[ $((sipstart)) == 1 ]]; then
				line=$(awk -F')<' '{print $2}' <<< "$line" | sed 's/> +$//g')
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "AAM" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"
...
#		nfiles=$(wc -w <<< "$filelist")
		...
		if [[ $((nfiles)) -gt 1 ]]; then
			echo -e "\nWarning: about to convert multiple files ($nfiles x CSTRACE*.txt)."

...
today=$(date +%m/%d/%Y)
year=$(date +%Y)
pattern1='^INFO.*REQUEST$|^INFO.*RESPONSE$'
pattern1x='^INFO.*REQUEST|^INFO.*RESPONSE'
pattern2='^FINE.*% $'
pattern2x='^FINE.*% '
pattern3='^% '
...
# TODO: AAWG logreport processing

function usage ()  {
    echo 'traceAAWG.sh v1.0 @ $year : converting SIP messages into a format required by traceSM tool'
	echo -e "\t\t\t\t\t\t\t    created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceAAWG.sh [OPTIONS] [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the CSA.log file from an AAWG or HCG server logreport"
	echo -e "\t\t\twhich can be located in /opt/Avaya/CallSignallingAgent/<release string>/logs/ folder"
	echo -e "\t<folder>\tincludes one or more of the log files extracted from logreport"	
# This log file can be found in /opt/Avaya/CallSignallingAgent/<release string>/logs/ folder
	echo '  Options:'
	echo -e "\t-h \t\tget Usage screen"
#	echo -e "\t-e \t\tconvert messages only with IP addr: a.b.c.d"	
	...
	echo ''
	echo " Note#1: AAWG may log a SIP msg without request header, eg. % OPTIONS. In such case, traceSM will show ERROR"
	echo " Note#2: AAWG does not log SIP messages with Rx/Tx addresses, hence sender/receiver will show 6.6.6.6:6666"
	echo ''	
} # usage()
...
function convert_siplog0 () {
	while IFS= read -r line
	do
		nlines=$((nlines+1))

		if [[ $((sipstart)) != 0 ]] && [[ $line =~ $pattern4 ]]; then
			complete_sipmsg
		fi
		if [[ $((insidesip)) == 0 ]] && [[ $line =~ $pattern1 ]]; then
			if [[ $endptaddr == "ACDC" ]]; then
		...
		elif [[ $((insidesip)) == 1 ]] && [[ $((dirdefined)) != 0 ]] && [[ $sipyear == "" ]]; then
		...
		elif [[ $((dirdefined)) != 0 ]] && [[ $sipyear != "" ]] && [[ $((sipstart)) == 0 ]]; then
			sipstart=1													# yes, there is an empty line !!

		elif [[ $((sipstart)) == 1 ]] && [[ $line =~ $pattern3 ]]; then
			...
			if [[ ${#line} != 0 ]]; then
#			if [[ $((insidesip)) == 1 ]] && [[ $((dirdefined)) != 0 ]] && [[ $((sipstart)) != 0 ]] && [[ $((siplines)) != 0 ]] && [[ $line =~ $pattern4 ]]; then
...
function convert_siplog1 () {
	while IFS= read -r line
	do
		nlines=$((nlines+1))
# if [[ $sipmsg -gt 50 ]]; then
# break
# fi
		if [[ $((sipstart)) != 0 ]] && [[ $line =~ $pattern4 ]]; then
			complete_sipmsg
		fi
		if [[ $((insidesip)) == 0 ]] && [[ $line =~ $pattern1x ]]; then
			if [[ $endptaddr == "ACDC" ]]; then
				if [[ $line != *$endptaddr* ]]; then	
					continue
				fi
		...
		elif [[ $((sipstart)) == 2 ]] && [[ ${#line} != 0 ]]; then
#			if [[ $((insidesip)) == 1 ]] && [[ $((dirdefined)) != 0 ]] && [[ $((sipstart)) != 0 ]] && [[ $((siplines)) != 0 ]] && [[ $line =~ $pattern4 ]]; then
			if [[ $line =~ $pattern4 ]]; then
				complete_sipmsg
			elif [[ $findANI != "" ]] && [[ $sipword =~ "INVITE" ]]; then
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -r -t1 CSA.log.1[0-9] 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
	...
	targetX=""; targetX=$(ls -t1 CSA.log.20 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
	...
	targetX=""; targetX=$(ls -r -t1 CSA.log.[1-9] 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
	...
	targetX=""; targetX=$(ls -t1 CSA.log 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
			targetfiles="$targetX $targetfiles"
		else
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [ -d "opt/Avaya/CallSignallingAgent" ]; then
		destdir="$destdir/opt/Avaya/CallSignallingAgent"
		target="$target-AAWG"
		cd "opt/Avaya/CallSignallingAgent"

	elif [ -d "Avaya/CallSignallingAgent" ]; then
		destdir="$destdir/Avaya/CallSignallingAgent"
		target="$target-AAWG"
		cd "Avaya/CallSignallingAgent"

	elif [ -d "CallSignallingAgent" ]; then
		destdir="$destdir/CallSignallingAgent"
		target="$target-AAWG"
		cd "CallSignallingAgent"
	fi

	reldir=""; reldir=$(ls -d -t1 [34].* 2>/dev/null | head -1)
	if [[ $? == 0 ]] && [[ $reldir != 0 ]]; then
		if [ -d "$reldir" ]; then
	...
	explore_logfolder
	if [[ $file == "" ]]; then
		error=1; echo -e "\nerror: could not find any AAWG related logs in $folder\n"
	fi
	cd "$currdir"	
else
...
if [[ $file != "" ]] && [ -s "$file" ]; then
	error=0; fsize=0; rec=0; rec2=0
	basefile=""; input3=""; bNoMatch=0
	...
	ftype=$(file -b "$file"); ftype2=$(file -bZ "$file")

	if [[ $ftype =~ compressed ]] && [[ $ftype2 =~ ASCII|text|data ]]; then
		if [[ $basefile == *"."* ]]; then
			input3=${basefile%.*}
		else
			input3="$basefile"
		...
		if [[ $bGunzip != 0 ]]; then
			echo "Uncompressing $basefile into $input3 ..."
			gunzip -q -c "$file" > "$input3" 2>/dev/null

			if [[ $? -le 1 ]]; then
				file="$input3"; tmpfile=2
			else
				echo -e "\nerror: failed to uncompress $basefile, using \"gunzip\" utility.\n"
				error=8; continue
			fi
		else
			echo -e "\nerror: unable to uncompress $basefile, \"gunzip\" utility not found.\n"
			error=8; continue
		fi
	fi
...
#	echo "                                                                                                                                                  "
	...
	rec2=$(egrep -c $pattern1 < "$file")
	if [[ $rec2 == 0 ]]; then
		bNoMatch=1; rec2=$(egrep -c $pattern1x < "$file")
	fi

	if [[ $rec2 == 0 ]]; then
		error=1	
		echo "error: No SIP messages have been found in $basefile in the expected format."
		echo "This file may not be an AAWG CSA.log, or DEBUG was not enabled."

		if [[ $rec == 0 ]]; then
			echo "In fact, no sign of any "CSeq:" lines within $basefile."
			error=2
		else
			echo "Though, found $rec lines with \"CSeq:\" - so there might be some SIP messages in $basefile"
		...
		echo "# Host: $HOSTNAME   PWD=$PWD   vsyslog=$vsyslog   bNoMatch=$bNoMatch" >> "$newfile"
...
#  	    conv=$(awk -e '/^INFO.*REQUEST$|^INFO.*RESPONSE$/{flag=1} flag; /}/{flag=0}' "$file")
		if [[ $bNoMatch == 0 ]]; then														# regular CSA matches with RESPONSE$
    	    conv=$(awk -W source='/^INFO.*REQUEST$|^INFO.*RESPONSE$/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
			if [[ $? != 0 ]]; then
    	    	conv=$(awk -e '/^INFO.*REQUEST$|^INFO.*RESPONSE$/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
			fi
			convert_siplog0
		else																				# have to take $ out from pattern
#  	    	conv=$(awk -W source='/^INFO.*REQUEST|^INFO.*RESPONSE/{flag=1} flag; /}/{flag=0}' "$file")
   	    	conv=$(awk -W source='/^INFO.*REQUEST/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)			
			if [[ $? != 0 ]]; then
				conv=$(awk -e '/^INFO.*REQUEST/{flag=1} flag; /}/{flag=0}' "$file" 2>/dev/null)
			fi
			convert_siplog1
		fi
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "AAWG" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"
...
#		nfiles=$(wc -w <<< "$filelist")
		...
		if [[ $((nfiles)) -gt 1 ]]; then
			echo -e "\nWarning: about to convert multiple files ($nfiles x CSA.log*.txt)."

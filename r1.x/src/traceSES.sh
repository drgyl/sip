...
function usage ()  {
    echo "traceSES.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
...
##			foundipaddr=$(echo $line | cut -d' ' -f5)
			sipyear=$(echo "$line" | awk '{print $4}')          # cut -d' ' -f4) --  because of multiple spaces
			sipday=$(echo "$line"  | awk '{printf "%02i",$2}')          # cut -d' ' -f2) --  because of multiple spaces
...
# May  4 19:59:47 2009 matching filter label <4601_BAD>: HELPDESK-SES.avaya.com: [Recv Request  ]
		sipmsec=$(echo "$line"  | awk '{print $3}')          # cut -d' ' -f4) --  because of multiple spaces
...
for var in "$@"
	...
	if [ -f $file ]; then
		...
		rec=$(egrep -c -e "^\{connection:" "$file")

		if [[ $rec == 0 ]];	then
			echo "error: No SIP messages have been found in $var. This file may not be an SeS sipTraceLog.txt logfile"
			...
			if [[ $rec == 0 ]]; then
				...
				echo "Though, found $rec lines with "CSeq:" - so there might be some sort of SIP messages within $var."
				rec=0
			fi
#            rec=$(egrep -c -m 1 -e "^Server: Avaya SIP Enablement Services")
			rec=$(egrep -c -m 1 -e "^Server:Avaya SIP Enablement Services.*")
			if [[ $rec == 0 ]]; then
			    echo "No indication $var being related to SES logfile."
			else
			    echo "Though, found reference in $var to SES."
			fi
			...
			do
				...
				if [[ $((insidesip)) == 0 ]] && [[ $line == *"matching filter"* ]]; then       # this is a new SIP msg
					...
					if [[ $line == *"Unavailable"* ]]; then
						...
						ip1=$(echo "$line" | cut -d'=' -f2 | cut -d' ' -f1)
						ip2=$(echo "$line" | cut -d'=' -f3 | cut -d' ' -f1)
						protocol=$(echo "$line" | cut -d'=' -f4 | cut -d'}' -f1)
						ip="$ip1:$ip2"
					fi

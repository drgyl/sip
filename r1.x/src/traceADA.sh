...
function usage ()  {
    echo "traceADA.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t    created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceADA.sh <options> [<LOG_FILE>,...]'
	echo '  Where:'
...
for var in "$@"
	...
	if [ -f "$file" ]; then
		...
		if [[ $rec == 0 ]];	then
			...
			echo "Perhaps this file is not a DeviceAdapter dsa.log file... or, DEBUG was not enabled"
			error=1
        ...
        if [[ $((sipstat)) != 0 ]]; then
			if [[ ${#endptaddr} == 0 ]]; then
				...
				if [[ $((sipmsg)) == 0 ]]; then 
					...
					echo "    have been converted for addr=$endptaddr into $newfile file"
				fi
			fi

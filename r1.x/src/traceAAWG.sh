...
function usage ()  {
    echo 'traceAAWG.sh v1.0 @ 2022 : converting SIP messages into a format required by traceSM tool'
	echo -e "\t\t\t\t\t\t\t   created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceAAWG.sh <options> [<LOG_FILE>,...]'
	echo '  Where:'
...
#	echo -e "\t-e:\t\tconvert messages only with IP addr: a.b.c.d"	
	...
	echo ''
	echo -e "Note#1: AAWG may log a SIP msg without request header - eg. % OPTIONS. In such case, traceSM will show ERROR"
	echo -e "Note#2: AAWG does not log SIP messages with Rx/Tx addresses, hence sender/receiver will show 6.6.6.6:6666"
	echo ''	
} # usage()
...
for var in "$@"
	...
	if [ -f $file ]; then
		...
		rec=$(egrep -c $pattern1 < "$file")

		if [[ $rec == 0 ]];	then
			echo "error:No SIP messages have been found in $var."
			echo "Perhaps this file is not an AAWG CSA.log, or DEBUG was not enabled."
...
#    	    conv=$(awk -e '/^INFO.*REQUEST$|^INFO.*RESPONSE$/{flag=1} flag; /}/{flag=0}' "$file")
			...
			do
				...
				if [[ $((sipstart)) == 1 ]] && [[ $line =~ $pattern4 ]]; then
					complete_sipmsg
				fi
				if [[ $((insidesip)) == 0 ]] && [[ $line =~ $pattern1 ]]; then
					...
					sipstart=1
				elif [[ $((sipstart)) != 0 ]] && [[ $line =~ $pattern3 ]]; then

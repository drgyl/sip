...
function usage ()  {
    echo "traceCBA.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t    created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceCBA.sh <options> [<LOG_FILE>,...]'
	echo '  Where:'
...
# time="1487794945966"
	...
	sipmsec=$(echo $sipyear  | cut -d' ' -f2)
	sipday=$(echo $sipyear | cut -d'/' -f2)
	sipmonth=$(echo $sipyear | cut -d'/' -f1)
	sipyear=$(echo $sipyear | cut -d'/' -f3 | cut -d' ' -f1)	
...
#		rec2=$(egrep -c -e "^CSeq:*" "$file")		
		...
		if [[ $rec == 0 ]];	then
			...
			echo "Perhaps this file is not a CBA BSR siptrace.log file... or, DEBUG was not enabled"
			error=1

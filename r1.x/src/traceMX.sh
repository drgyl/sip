...
function usage ()  {
    echo "traceMX.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t   created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceMX.sh <options> [<LOG_FILE>,...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the rolling.sip.log file from an MX server SIPAGENT logreport"
	echo '  Options:'
	...
	echo ''
	echo "Note: rolling.sip.log misses to log any dates, hence in conversion applying current date."
	echo ''
} # usage()
...
####		siphour=$(echo $siphour |cut -d ':' -f1) # awk -F ':' '{print $1}')
		...
		sipmsec=$(echo $sipmsec   | cut -d':' -f3 | cut -d',' -f2)
...
for var in "$@"
	...
	if [ -f $file ]; then
		...
		if [[ $rec == 0 ]];	then
			echo "error: No SIP messages have been found in $file in the expected format."
			echo "Perhaps $file is not an MX rolling.sip.log file... or, DEBUG loglevel was not enabled."
			...
			if [[ $rec == 0 ]]; then
				...
				echo "Though, found $rec lines with "CSeq:" - so there might be some sort of SIP messages in $file."
				rec=0
			fi
			...
			do
				...
				if [[ $((dirdefined)) == 0 ]] && [[ $line == *"Sigtrace"* ]]; then
					...
					ip1=$(echo "$line" | cut -d':' -f4 | cut -d'[' -f2)		# TCP IPv4 Local:[10.174.227.106:5060], Remote:[10.180.19.104:41129]
					ip2=$(echo "$line" | cut -d':' -f5 | cut -d']' -f1)
					ip=$ip1:$ip2
					ip1=$(echo "$line" | cut -d':' -f2 | cut -d'[' -f2)		# 000000000000> INCOMING (10.20.32.81:36143) TCP
					ip2=$(echo "$line" | cut -d':' -f3 | cut -d']' -f1)
					localip=$ip1:$ip2

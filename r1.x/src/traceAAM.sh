...
function usage ()  {
    echo "traceAAM.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t    created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceAAM.sh <options> [<LOG_FILE>,...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis a CSTRACE file from AAM server logreport, located in /var/log/avaya/messaging/cs folder"
	echo '  Options:'
...
# 12/07 11:32:53.469802 CH061: SIP: call: SG::OnSipIncomgMsg sipio(i)<SIP/2.0 100 Trying                                          > len=429 from=10.230.1.14:5060	
	if [[ $line == *"OnSipIncomgMsg"* ]]; then
		...
		siplength=$(echo "$line" | awk -F'len=' '{print $2}' | cut -d' ' -f1)
...
#			sipyear=$(echo $today     | cut -d'/' -f3)
			...
			sipday=$(echo $sipmonth   | cut -d'/' -f2)
			sipmonth=$(echo $sipmonth | cut -d'/' -f1)					
##		fi
...
####		siphour=$(echo $siphour |cut -d ':' -f1) # awk -F ':' '{print $1}')
		...
		sipmsec=$(echo $sipsec  | awk -F'.' '{printf "%3i",$2/1000}')
...
for var in "$@"
	...
	if [ -f "$file" ]; then
		...
		if [[ $rec == 0 ]];	then
			echo "error: No SIP messages have been found in $var. Perhaps this file is not an AAM CSTRACE logfile..."
			error=1
...
#			conv=$(awk -e '/.*OnSipIncomgMsg.*|.*SendToNetwork.*/{flag=1} flag; /}/{flag=0}' "$file")
			...
			do
				...
				if [[ $((dirdefined)) == 0 ]]; then
					...
					line=$(echo "$line" | awk -F')<' '{print $2}' | sed 's/> +$//g')

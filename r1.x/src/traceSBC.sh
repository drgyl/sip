...
function usage ()  {
    echo "traceSBC.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
...
if [[ $((dirdefined)) == 0 ]]; then		
	...
	if [[ $((dirdefined)) == 2 ]]; then
	   ...
	   localip=$(echo "$line"  | cut -d' ' -f3)
	   protocol=$(echo "$line" | awk '{printf substr($6,2,3)'})   # cut -d' ' -f6 | cut -d'(' -f2 | cut -d')' -f1)
#	   protocol=$(echo "$line" | cut -d' ' -f6 | cut -d'(' -f2 | cut -d')' -f1)	   
	elif [[ $((dirdefined)) == 1 ]]; then
       localip=$(echo "$line"  | cut -d' ' -f5)
...
if [[ $# -eq 0 ]]; then
  ...
  while getopts ":hbf:se:" options; do
	case "${options}" in
	h)
		usage
		exit 0;;
...
for var in "$@"
	...
	if [ -f $file ]; then
		...
		rec=$(egrep -c -e "^SIP IN:.*|^SIP OUT:.*" "$file")

		if [[ $rec == 0 ]];	then
			echo "error: no SIP messages have been found in $var."
			echo "Perhaps this file is not an SBC SIP log file... or, FINEST debug was not enabled"
			error=1

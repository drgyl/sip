...
function usage ()  {
    echo "traceXT.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t   created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceXT.sh <options> [<LOG_FILE>,...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the user.log file from the logreport of an XT video endpoint"
	echo '  Options:'
...
#			sipyear=$(echo "$line"   | cut -d' ' -f1 | cut -d'+' -f1)
			...
			sipsec=$(echo "$sipyear" | cut -d'T' -f2) 
#			sipday=$(echo $sipyear   | cut -d'-' -f3 | cut -d'T' -f1)
            sipday=$(echo $sipyear   | cut -d'T' -f1 | cut -d'-' -f3)
...
for var in "$@"
	...
	if [ -f $file ]; then
		...
		if [[ $rec == 0 ]]; then
			echo "This file does not appear to be an user.log file from an XT device..."
		else
...
#			rec=$(egrep -c -e ".*recv <--.*|.*message sent to.*" < $file)
			...
			if [[ $rec == 0 ]]; then
				echo "No SIP messages have been found in $var."
				echo "Perhaps this file is not an user.log file from XT device..."

...
function usage ()  {
    echo "traceRC.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t   created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceRC.sh [<LOG_FILE>,...]'
	echo '  Where:'
...
if [[ $((dirdefined)) == 0 ]]; then		
	...
	if [[ $((dirdefined)) != 0 ]]; then
       ip1=$(echo "$line"    | cut -d'|' -f5 | cut -d'[' -f2)
	   siplength=$(echo $ip1 | cut -d' ' -f2 | cut -d'(' -f2 | cut -d')' -f1)
	   protocol=$(echo $ip1  | cut -d']' -f1 | cut -d':' -f1 )
	   ip2=$(echo $ip1 | cut -d']' -f1 | cut -d':' -f3)
...
for var in "$@"
	...
	if [ -f $file ]; then
		...
		n=$(egrep -c -m 1 -e "SIP_MSG"  "$file")

		if [[ $rec == 0 ]];	then
			echo "error: No SIP messages have been found in $var."
			echo "Perhaps this file is not an smc.txt logfile from RingCentral... or, FINEST debug was not enabled"
			echo ''
			error=2; continue
		elif [[ $n == 0 ]]; then
	        echo "error: $var does not appear to be an RC smc logfile. Verify source and content"
			echo ''			
			error=1; continue
		else		
			base64found=0

...
function usage ()  {
    echo "tracePhoner.sh v$version @ 2022 : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t       created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: tracePhoner.sh [<LOG_FILE>,...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the debugSIP.txt logfile (collected from Phoner or PhonerLite 3rd-party softclient)"	
	echo '  Options:'
...
if [[ $((dirdefined)) == 0 ]]; then		
	...
	if [[ $((dirdefined)) != 0 ]]; then
	   ...
	   protocol=$(echo "$line" | cut -d' ' -f4 | cut -d'(' -f2 | cut -d')' -f1)
	fi
fi	
} # sip_direction()
...
# 09:36:24,060: T: 135.64.253.72:5060 (TCP)
...
for var in "$@"
	...
	if [ -f $file ]; then
		...
		rec=$(egrep -m 1 -c -e "*honer[L: ].*" "$file")
		if [[ $rec == 0 ]]; then
			echo "error: $var does not appear to be a Phoner or PhonerLite log file."
			error=1; continue
		else
			rec=$(egrep -c -e "^CSeq:.*" "$file")

			if [[ $rec == 0 ]];	then
				echo "error: No SIP messages have been found in $var. Verify source and content of this file."
...
#				if [[ $line =~ $pattern1 ]] && [[ $line =~ \([A-Z]{3}\) ]]; then
				#  && [[ $line =~ $pattern2 ]]
				   if [[ $((sipstart)) != 0 ]]; then
                      complete_sipmsg
				   fi

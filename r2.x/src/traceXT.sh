...
vsyslog=12  ## values explained below:

# TODO: handle CS_package, which can include multip user.log files

function usage ()  {
    echo "traceXT.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t     created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceXT.sh <options> [<LOG_FILE> <folder>, ...]'
	echo '  Where:'
	echo -e "\t<LOG_FILE>\tis the xt-user.log file from the logreport of an XT video endpoint"
	...
	echo ''
	echo -e "  Note:\t\t\tthis device does not log SIP messages in its completeness, also src/dest is unknown"
	echo ''
} # usage()
...
#		dirdefined=1
		...
		ip="6.6.6.6:666"; 		siplength="666"; 		protocol="TLS"
...
#		dirdefined=2
		...
		if [[ $((dirdefined)) == 3 ]]; then
			ip=$(awk -F'sent to ' '{print $2}' <<< "$line")
			protocol=$(cut -d'=' -f2 <<< "$ip" | cut -d'.' -f1)
			ip=$(cut -d',' -f1 <<< "$ip")
#			dirdefined=2
		else
			ip="6.6.6.6:666"; 		protocol="TLS"
		fi
		siplength="666"
	else
	    reset_sipmsg
...
#			sipyear=$(echo "$line"   | cut -d' ' -f1 | cut -d'+' -f1)
			sipyear=$(cut -d'+' -f1 <<< "$line")
			sipsec=$(cut -d'T' -f2 <<< "$sipyear")
#			sipday=$(echo $sipyear   | cut -d'-' -f3 | cut -d'T' -f1)
            sipday=$(cut -d'T' -f1 <<< "$sipyear" | cut -d'-' -f3)
...
#		sipsec=$(echo "$line"   | cut -d' ' -f2) 
		...
		sipsec=$(cut -d':' -f3 <<< "$sipsec" | cut -d'.' -f1)
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls xt-user.log.[1-9] 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
	...
	targetX=""; targetX=$(ls -t1 xt-user.log 2>/dev/null)
	if [[ $? == 0 ]] && [[ $targetX != "" ]]; then
		if [[ $targetfiles != "" ]]; then
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	explore_logfolder
	if [[ $file == "" ]]; then
		echo -e "\nerror: could not find any XT related logs in $folder\n"
		error=1
	fi
	cd $currdir
else
...
#	echo "                                                                                                                                                  "
	...
	rec=$(egrep -c -m 1 ".*\(call_index -1\) recv <--.*|.*\(call_index -1\) sent -->.*" "$file")

	if [[ $rec == 0 ]]; then
		echo -e "\nThis file $basefile does not appear to be an xt-user.log file from an XT device..."
		...
		useragent=$(egrep -m 1 -e "*Product = *" "$file" | awk -F"Product = " '{print $2}')

		if [[ $useragent =~ XT ]]; then
		...
		egrep "SUAS\||CORE\|" < "$file" > "$file.tmp"
		file="$file.tmp"; tmpfile=2
#		rec=$(egrep -c -e ".*recv <--.*|.*message sent to.*" < $file)
		rec=$(egrep -c -e ".*\(call_index -1\) recv <--.*|.*\(call_index -1\) sent -->.*" "$file")

		if [[ $rec == 0 ]]; then
			echo "No SIP messages have been found in $basefile."
			echo "Perhaps this file is not an xt-user.log file from XT device..."
	...
	if [[ $rec != 0 ]] && [[ $((vsyslog)) != 0 ]]; then
		...
		do
			nlines=$((nlines+1))
# 2021-03-18T11:12:15.027001+00:00 vdocom: HOST|VVoIP |NOR|m00000| SUAS| Incoming call from remote IP 37.49.229.140 (local IP=10.151.3.200, local interface=LAN).
			if [[ $((sipstart)) != 0 ]]; then
				linex=""
				if [[ $line =~ SUAS\| ]]; then
					linex=$(awk -F'SUAS' '{print $2}' <<< "$line" | tr '|' '+' | awk -F'+ ' '{print $2}')
#					linex=$(echo "$line" | awk -F'SUAS' '{print $2}' | tr '|' '+' | sed 's/^+[[:blank:]]*//' )
				elif [[ $line =~ CORE\| ]]; then
					linex=$(awk -F'CORE' '{print $2}' <<< "$line" | tr '|' '+' | awk -F'+ ' '{print $2}')
				else
					complete_sipmsg
					continue
				fi

				if [[ $linex == *"recv <--"* ]] || [[ $linex == *"sent -->"* ]]; then
					if [[ $line =~ SUAS\||CORE\| ]]; then
						complete_sipmsg
					fi
				# elif [[ $line == *"SUAS| SIP"* ]] || [[ $line == *"SUAS| ( "* ]]; then
				else
					nmatch=$(egrep -c -e "^[A-Z(].*" <<< "$linex")
# echo linex=$nmatch-$linex===$line
#					elif [[ echo $linex | egrep -e "| [AST(" ]]; then
					if [[ $((nmatch)) != 0 ]]; then
						complete_sipmsg
						continue
					else
					    line=$(sed 's/^[[:blank:]]*//' <<< "$linex")
					fi
				fi				
			fi			

			if [[ $((insidesip)) == 0 ]] && [[ $line =~ SUAS\| ]]; then
				if [[ $line =~ H323 ]]; then
					continue
				elif [[ $line == *"recv <--"* ]]; then
...
# echo DIR1=$siptotalmsg $line
					...
					line=$(awk -F'--> ' '{print $2}' <<< "$line")
					start_sipmsg
# echo DIR2=$siptotalmsg $line					
				fi

#			elif [[ $((dirdefined)) == 3 ]] && [[ $line == *"sent -->"* ]]; then         # TODO: when can 3 occur?? see CU360
#				if [[ $line =~ H323 ]]; then
#					reset_sipmsg
#				else
#					dirdefined=2
...
if [[ $# -eq 0 ]]; then
  ...
  while getopts ":hbdf:sACN:" options; do
	case "${options}" in
	h)
		usage
		exit 0;;
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "XT" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"
...
#		nfiles=$(wc -w <<< "$filelist")
		...
		if [[ $((nfiles)) -gt 1 ]]; then
			echo -e "\nWarning: about to convert multiple files ($nfiles x user.log* logfiles)."

...
function usage ()  {
	...
	echo -e "\t-I\ t\tignore all SIP INFO messages (used in sharedcontrol session, DTMF or in CCMSoverSIP)"
	echo -e "\t-N ANI\t\tfind a call with caller/called number matching to ANI (digit string) in To: or From:"
...
#		elif [[ $sipmsghdr != "" ]] && [[ $sipmsghdr =~ .*CSeq:\ [0-9]+\ INFO* ]]; then
            ...
            infox=$(egrep -m 1 -c ".*CSeq:\ [0-9]+\ INFO*" <<< "$linex")               # this is for 200OK INFO
            if [[ $((infox)) != 0 ]]; then
			    infofound=1
            fi
		fi
...
#		get_useragent2    
		...
		if [[ $findANI != "" ]] && [[ $sipword =~ "INVITE" ]]; then
			if [[ $calltime == "" ]]; then
				linefrom=$(egrep -e "^From:|^To:" <<< "$linex")
				lineid=$(egrep -e "^Call-ID:" <<< "$linex")
...
if [[ $# -eq 0 ]]; then
  ...
  while getopts ":e:hbf:Ss:p:N:I" options; do
	case "${options}" in
	h)
		usage
		exit 0;;
...
# echo sharkfilter=$sharkfilter e=$endptaddr s=$srvraddr			
...
#        	    	rec=$(egrep -c -e ".*CSeq:.*" "$sipfile")
					if [[ $rec != $nlines ]] || [[ $rec2 == 0 ]]; then
						echo ''; error=1
						echo rec=$rec  rec2=$rec2  nlines=$nlines
						echo "error: could not extract any valid SIP messages from this wireshark capture file: $bvar"
						if [[ $((rec2)) -gt 0 ]]; then
			...
			echo -e "\nerror: $var does not appear to be a wireshark capture file."
			echo -e "Verify source and content of $bvar.\n"
			error=1; continue
        fi
...
#			echo ''
			...
			do
				...
				if [[ $line =~ ^[0-9]+\#[0-9]+\.[0-9]{9}\#.* ]]; then
					...
					if [[ $dirdefined == 0 ]]; then
						reset_sipmsg; insidemsg=0; continue
...
#						explore_sipcont
        	...
        	if [[ $((sipstat)) != 0 ]]; then
				if [[ ${#endptaddr} == 0 ]]; then
					echo "==> $sipmsg out of $n/$rec SIP messages have been converted into $var.asm"
				else
					if [[ $((sipmsg)) == 0 ]]; then 
				...
				echo -e "\n\tTotal # of lines digested:\t\t\t $nlines"
			...
			echo '' >> "$newfile"
		    if [[ $sipwordlist != "" ]]; then
			   echo -e "# SIP status/requests found: $sipwordlist" >> "$newfile"
		    fi

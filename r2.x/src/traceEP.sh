...
function usage ()  {
    echo "traceEP.sh v$version @ $year : converting SIP messages into a format required by traceSM tool"
	echo -e "\t\t\t\t\t\t\t     created by <lgyalog@avaya.com>"	
#	echo ''
	echo 'Usage: traceEP.sh <options> [<LOG_FILE> | <folder>, ...]'
	echo '  Where:'
...
if [[ $((dirdefined)) == 0 ]]; then		
	...
	if [[ $((dirdefined)) == 1 ]]; then
       ip=$(cut -d' ' -f4 <<< "$line" | sed 's/^src=*//g')
	   localip=$(cut -d' ' -f5 <<< "$line" | sed 's/^dst=*//g')
    elif [[ $((dirdefined)) == 2 ]]; then
       localip=$(cut -d' ' -f4 <<< "$line" | sed 's/^src=*//g')
	   ip=$(cut -d' ' -f5 <<< "$line" | sed 's/^dst=*//g')
	fi
fi	
} # sip_direction()
...
function explore_logfolder() {
	...
	targetX=""; targetX=$(ls -t1 SessionManager[1-9].log 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		targetfiles=$targetX
	...
	targetX=""; targetX=$(ls -t1 SessionManager.log 2>/dev/null)
	if [[ $? != 0 ]]; then
		targetX=""
	elif [[ $targetX != "" ]]; then
		if [[ "$targetfiles" != "" ]]; then
...
if [[ $folder != "" ]] && [[ $destdir != "" ]]; then
	...
	if [ -d "opt/Avaya/ExperiencePortal/MPP/logs/process/SessMgr" ]; then
		destdir="$destdir/opt/Avaya/ExperiencePortal/MPP/logs/process/SessMgr"
		target="$target-EP"
		cd "opt/Avaya/ExperiencePortal/MPP/logs/process/SessMgr"
	elif [ -d "Avaya/ExperiencePortal/MPP/logs/process/SessMgr" ]; then
		destdir="$destdir/Avaya/ExperiencePortal/MPP/logs/process/SessMgr"
		target="$target-EP"
		cd "Avaya/ExperiencePortal/MPP/logs/process/SessMgr"
	elif [ -d "ExperiencePortal/MPP/logs/process/SessMgr" ]; then
		destdir="$destdir/ExperiencePortal/MPP/logs/process/SessMgr"
		target="$target-EP"
		cd "ExperiencePortal/MPP/logs/process/SessMgr"
	elif [ -d "MPP/logs/process/SessMgr" ]; then
		destdir="$destdir/MPP/logs/process/SessMgr"
		target="$target-EP"
		cd "MPP/logs/process/SessMgr"
	elif [ -d "logs/process/SessMgr" ]; then
		destdir="$destdir/logs/process/SessMgr"
		target="$target-EP"
		cd "logs/process/SessMgr"
	elif [ -d "process/SessMgr" ]; then
		destdir="$destdir/process/SessMgr"
		target="$target-EP"
		cd "process/SessMgr"
	elif [ -d "SessMgr" ]; then
		destdir="$destdir/SessMgr"
		target="$target-EP"
		cd "SessMgr"
	...
	explore_logfolder
	if [[ $file == "" ]]; then
		echo -e "\nerror: could not find any ExperiencePortal related logs in $folder\n"
		error=1
	fi
	cd $currdir
else
...
#	echo "                                                                                                                                                  "
	...
	rec=$(egrep -c -e ".*\|SIP\|.*FileName=sip.*" "$file")

	if [[ $rec == 0 ]];	then
		echo -e "\nerror: No SIP messages have been found in $basefile in the expected format."
		echo "This file may not be an ExperiencePortal SessionManager.log file... or, FINEST debug was not enabled."
		...
		if [[ $rec -gt 500 ]]; then 
			...
			echo 0e "You may want to execute this script on a more powerful PC or server.\n"
		fi
			   	...
			   	if [[ $line == *"|SIP|.*FileName=sip"* ]]; then
					...
					if [[ $ip != "" ]] && [[ $endptaddr != "" ]] && [[ $ip != *$endptaddr* ]]; then
						if [[ $localip != "" ]] && [[ $endptaddr != "" ]] && [[ $localip != *$endptaddr* ]]; then
				 		...
				 		get_sip_datetime
						sipmsg_header
        	            line=$(awk -F"dst=" '{print $2}' <<< "$line" | cut -d'<' -f2 | sed 's/^\^*//g')
						start_sipmsg
					fi
				fi
...
#	target=${target%%.*}										# TODO: what about ../folder or ../filename - note the leading ".."	
	...
	if [ -d "$var" ]; then
		...
		elif [[ $filetype == "Zip archive"* ]] && [[ $filecontent == "EP" ]]; then
			if [[ $bvar == *"."* ]]; then
				input=${bvar%.*}
			else
				input="$bvar"
...
#		nfiles=$(wc -w <<< "$filelist")
		...
		if [[ $((nfiles)) -gt 1 ]]; then
			echo -e "\nWarning: about to convert multiple files ($nfiles x SessionManager.log.txt)."

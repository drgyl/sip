...
#		conv=$(awk -e '/.*OnSipIncomgMsg.*|.*SendToNetwork.*/{flag=1} flag; /}/{flag=0}' "$file")
		conv=$(awk -W source='/.*OnSipIncomgMsg.*|.*SendToNetwork.*/{flag=1} flag; /}/{flag=0}' "$file")			
		...
		do
			...
			if [[ $((sipstart)) != 0 ]]; then
				if [[ $line != *"OnSipIncomgMsg"* ]] && [[ $line != *"SendToNetwork"* ]] || [[ $line == *"> len="* ]]; then 
					complete_sipmsg
				fi
			fi
